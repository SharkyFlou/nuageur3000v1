...
class nuageMot {
  ...
  //renvoie le mot modifie, grace a l'etape 1
  public static string remplacement(string Xmot, string Xtermi, string Xparam) {
    string nvmot = "";
    for (int a = 0; a < (Xmot.Length - Xtermi.Length); a++) { //creer le nouveau mot sans la terminsaison, un substring marcherait aussi
      nvmot += Xmot[a];
    }
    ...
    foreach(KeyValuePair < string, int > val in dicOG) {
      ...
      for (int i = 0; i < liste_terminaison1.Count && !trouve; i++) {
        ...
        if (comparaison(val.Key, liste_terminaison1[i][1])) {
          ...
          racine = remplacement(val.Key, liste_terminaison1[i][1], liste_terminaison1[i][2]);
          trouve = true;
        }
      }
