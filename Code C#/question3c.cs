...
class nuageMot {
  ...
  public static List < string[] > terminaison(string filepath) {
    ...
    //lecture du fichier + addition e la lsite
    while (!(sr.EndOfStream)) {
      ligne = sr.ReadLine();
      string[] lignedecoupe = ligne.Split(" ");
  ...
  //transforme en racine, si possible
  ...
  //fonction qui afficge la liste avec toutes les racines et ses versions
  public static void afficheListMot(List < mot > XlisteM) {
    ...
    for (int i = XlisteM.Count - 1; i >= 0; i--) {
      Console.WriteLine("##############################");
      Console.WriteLine("Racine : " + XlisteM[i].racine);
      total = 0;
      max = 0;
      foreach(string val in XlisteM[i].version.Keys) {
        Console.Write("<" + val + "; nbr : " + XlisteM[i].version[val] + "> ");
        tempmax = XlisteM[i].version[val];
        if (tempmax > max) {
          max = tempmax;
          motmax = val;
        }
        total += XlisteM[i].version[val];
      }
      Console.WriteLine("\nTotal : " + total + " \t plus courant : " + motmax);
    }
  }

  //trie la liste finale pour une meilleure comprehension, et ne garde que les 50 premier
